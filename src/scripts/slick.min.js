(function(factory) {
  ...
  Slick = (function() {
    ...
    function Slick(element, settings) {
      ...
      }; $.extend(_, _.initials); _.activeBreakpoint = null; _.animType = null; _.animProp = null; _.breakpoints = []; _.breakpointSettings = []; _.cssTransitions = false; _.focussed = false; _.interrupted = false; _.hidden = 'hidden'; _.paused = true; _.positionProp = null; _.respondTo = null; _.rowCount = 1; _.shouldClick = true; _.$slider = $(element); _.$slidesCache = null; _.transformType = null; _.transitionType = null; _.visibilityChange = 'visibilitychange'; _.windowWidth = 0; _.windowTimer = null; dataSettings = $(element).data('slick') || {}; _.options = $.extend({}, _.defaults, settings, dataSettings); _.currentSlide = _.options.initialSlide; _.originalSettings = _.options;
  ...
  Slick.prototype.addSlide = Slick.prototype.slickAdd = function(markup, index, addBefore) {
    ...
    }_.$slides = _.$slideTrack.children(this.options.slide); _.$slideTrack.children(this.options.slide).detach(); _.$slideTrack.append(_.$slides);
  ...
  Slick.prototype.getNavTarget = function() {
    const _ = this; let asNavFor = _.options.asNavFor;
  ...
  Slick.prototype.asNavFor = function(index) {
    const _ = this; const asNavFor = _.getNavTarget();
  ...
  Slick.prototype.autoPlayIterator = function() {
    const _ = this; let slideTo = _.currentSlide + _.options.slidesToScroll;
  ...
  Slick.prototype.buildArrows = function() {
    ...
    if (_.options.arrows === true) {
      _.$prevArrow = $(_.options.prevArrow).addClass('slick-arrow'); _.$nextArrow = $(_.options.nextArrow).addClass('slick-arrow');
  ...
  Slick.prototype.buildDots = function() {
    const _ = this; let i; let dot;

    if (_.options.dots === true && _.slideCount > _.options.slidesToShow) {
      _.$slider.addClass('slick-dotted'); dot = $('<ul />').addClass(_.options.dotsClass);
  ...
  Slick.prototype.buildOut = function() {
    ...
    _.$slides = _.$slider.children(_.options.slide + ':not(.slick-cloned)').addClass('slick-slide'); _.slideCount = _.$slides.length;

    _.$slides.each(function(index, element) {
      $(element).attr('data-slick-index', index).data('originalStyling', $(element).attr('style') || '');
    }); _.$slider.addClass('slick-slider'); _.$slideTrack = _.slideCount === 0 ? $('<div class="slick-track"/>').appendTo(_.$slider) : _.$slides.wrapAll('<div class="slick-track"/>').parent(); _.$list = _.$slideTrack.wrap('<div class="slick-list"/>').parent(); _.$slideTrack.css('opacity', 0);
  ...
  Slick.prototype.buildRows = function() {
    ...
    if (_.options.rows > 0) {
      slidesPerSection = _.options.slidesPerRow * _.options.rows; numOfSlides = Math.ceil(originalSlides.length / slidesPerSection);
      ...
      _.$slider.children().children().children().css({
        width: 100 / _.options.slidesPerRow + '%', display: 'inline-block',
      });
    }
  };

  Slick.prototype.checkResponsive = function(initial, forceUpdate) {
    const _ = this; let breakpoint; let targetBreakpoint; let respondToWidth; let triggerBreakpoint = false; const sliderWidth = _.$slider.width(); const windowWidth = window.innerWidth || $(window).width();
    ...
    if (_.options.responsive && _.options.responsive.length && _.options.responsive !== null) {
      ...
      if (targetBreakpoint !== null) {
        ...
        if (_.activeBreakpoint !== null) {
          _.activeBreakpoint = null; _.options = _.originalSettings;
  ...
  Slick.prototype.changeSlide = function(event, dontAnimate) {
    const _ = this; let $target = $(event.currentTarget); let indexOffset; let slideOffset; let unevenOffset;
    ...
    switch (event.data.message) {
      case 'previous':slideOffset = indexOffset === 0 ? _.options.slidesToScroll : _.options.slidesToShow - indexOffset;

        if (_.slideCount > _.options.slidesToShow) {
          _.slideHandler(_.currentSlide - slideOffset, false, dontAnimate);
        } break; case 'next':slideOffset = indexOffset === 0 ? _.options.slidesToScroll : indexOffset;

        if (_.slideCount > _.options.slidesToShow) {
          _.slideHandler(_.currentSlide + slideOffset, false, dontAnimate);
        } break; case 'index':var index = event.data.index === 0 ? 0 : event.data.index || $target.index() * _.options.slidesToScroll;

        _.slideHandler(_.checkNavigable(index), false, dontAnimate); $target.children().trigger('focus'); break; default:
    }
  };

  Slick.prototype.checkNavigable = function(index) {
    const _ = this; let navigables; let prevNavigable;

    navigables = _.getNavigableIndexes(); prevNavigable = 0;
  ...
  Slick.prototype.cleanUpEvents = function() {
    ...
    if (_.options.arrows === true && _.slideCount > _.options.slidesToShow) {
      ...
      if (_.options.accessibility === true) {
        _.$prevArrow && _.$prevArrow.off('keydown.slick', _.keyHandler); _.$nextArrow && _.$nextArrow.off('keydown.slick', _.keyHandler);
      }
    }_.$list.off('touchstart.slick mousedown.slick', _.swipeHandler); _.$list.off('touchmove.slick mousemove.slick', _.swipeHandler); _.$list.off('touchend.slick mouseup.slick', _.swipeHandler); _.$list.off('touchcancel.slick mouseleave.slick', _.swipeHandler); _.$list.off('click.slick', _.clickHandler); $(document).off(_.visibilityChange, _.visibility); _.cleanUpSlideEvents();
  ...
  Slick.prototype.cleanUpSlideEvents = function() {
    ...
    _.$list.off('mouseenter.slick', $.proxy(_.interrupt, _, true)); _.$list.off('mouseleave.slick', $.proxy(_.interrupt, _, false));
  };
  ...
  Slick.prototype.destroy = function(refresh) {
    ...
    }_.cleanUpRows(); _.$slider.removeClass('slick-slider'); _.$slider.removeClass('slick-initialized'); _.$slider.removeClass('slick-dotted'); _.unslicked = true;
  ...
  Slick.prototype.getDotCount = function() {
    const _ = this; let breakPoint = 0; let counter = 0; let pagerQty = 0;
  ...
  Slick.prototype.getLeft = function(slideIndex) {
    const _ = this; let targetLeft; let verticalHeight; let verticalOffset = 0; let targetSlide; let coef;
  ...
  Slick.prototype.getNavigableIndexes = function() {
    const _ = this; let breakPoint = 0; let counter = 0; const indexes = []; let max;
  ...
  Slick.prototype.getSlideCount = function() {
    const _ = this; let slidesTraversed; let swipedSlide; let swipeTarget; let centerOffset;

    centerOffset = _.options.centerMode === true ? Math.floor(_.$list.width() / 2) : 0; swipeTarget = _.swipeLeft * -1 + centerOffset;

    if (_.options.swipeToSlide === true) {
      _.$slideTrack.find('.slick-slide').each(function(index, slide) {
        let slideOuterWidth, slideOffset, slideRightBoundary;

        slideOuterWidth = $(slide).outerWidth(); slideOffset = slide.offsetLeft;
  ...
  Slick.prototype.initADA = function() {
    const _ = this; const numDotGroups = Math.ceil(_.slideCount / _.options.slidesToShow); const tabControlIndexes = _.getNavigableIndexes().filter(function(val) {
    ...
    if (_.$dots !== null) {
      _.$slides.not(_.$slideTrack.find('.slick-cloned')).each(function(i) {
        ...
        $(this).attr({
          role: 'tabpanel', id: 'slick-slide' + _.instanceUid + i, tabindex: -1,
        });
  ...
  Slick.prototype.lazyLoad = function() {
    ...
    function loadImages(imagesScope) {
      $('img[data-lazy]', imagesScope).each(function() {
        const image = $(this); const imageSource = $(this).attr('data-lazy'); const imageSrcSet = $(this).attr('data-srcset'); const imageSizes = $(this).attr('data-sizes') || _.$slider.attr('data-sizes'); const imageToLoad = document.createElement('img');
    ...
    if (_.options.lazyLoad === 'anticipated') {
      let prevSlide = rangeStart - 1; let nextSlide = rangeEnd; const $slides = _.$slider.find('.slick-slide');
  ...
  Slick.prototype.loadSlider = function() {
    ...
    _.setPosition(); _.$slideTrack.css({ opacity: 1 }); _.$slider.removeClass('slick-loading'); _.initUI();
  ...
  Slick.prototype.pause = Slick.prototype.slickPause = function() {
    ...
    _.autoPlayClear(); _.paused = true;
  };

  Slick.prototype.play = Slick.prototype.slickPlay = function() {
    const _ = this;

    _.autoPlay(); _.options.autoplay = true; _.paused = false; _.focussed = false; _.interrupted = false;
  };
  ...
  Slick.prototype.postSlide = function(index) {
    ...
    if (!_.unslicked) {
      _.$slider.trigger('afterChange', [_, index]); _.animating = false;
  ...
  Slick.prototype.progressiveLazyLoad = function(tryCount) {
    ...
    const _ = this; const $imgsToLoad = $('img[data-lazy]', _.$slider); let image; let imageSource; let imageSrcSet; let imageSizes; let imageToLoad;

    if ($imgsToLoad.length) {
      image = $imgsToLoad.first(); imageSource = image.attr('data-lazy'); imageSrcSet = image.attr('data-srcset'); imageSizes = image.attr('data-sizes') || _.$slider.attr('data-sizes'); imageToLoad = document.createElement('img');
  ...
  Slick.prototype.refresh = function(initializing) {
    ...
    if (_.slideCount <= _.options.slidesToShow) {
      _.currentSlide = 0;
    }currentSlide = _.currentSlide; _.destroy(true); $.extend(_, _.initials, { currentSlide: currentSlide }); _.init();
  ...
  Slick.prototype.registerBreakpoints = function() {
    const _ = this; let breakpoint; let currentBreakpoint; let l; const responsiveSettings = _.options.responsive || null;
  ...
  Slick.prototype.resize = function() {
    ...
    if ($(window).width() !== _.windowWidth) {
      ...
      _.windowDelay = window.setTimeout(function() {
        _.windowWidth = $(window).width(); _.checkResponsive();
  ...
  Slick.prototype.setCSS = function(position) {
    const _ = this; let positionProps = {}; let x; let y;

    if (_.options.rtl === true) {
      position = -position;
    }x = _.positionProp == 'left' ? Math.ceil(position) + 'px' : '0px'; y = _.positionProp == 'top' ? Math.ceil(position) + 'px' : '0px'; positionProps[_.positionProp] = position;
  ...
  Slick.prototype.setOption = Slick.prototype.slickSetOption = function() {
    ...
    if ($.type(arguments[0]) === 'object') {
      ...
      option = arguments[0]; value = arguments[1]; refresh = arguments[2];
  ...
  Slick.prototype.setProps = function() {
    const _ = this; const bodyStyle = document.body.style;
    ...
    if (bodyStyle.MozTransform !== undefined) {
      _.animType = 'MozTransform'; _.transformType = '-moz-transform'; _.transitionType = 'MozTransition';
    ...
    if (bodyStyle.webkitTransform !== undefined) {
      _.animType = 'webkitTransform'; _.transformType = '-webkit-transform'; _.transitionType = 'webkitTransition';
    ...
    if (bodyStyle.msTransform !== undefined) {
      _.animType = 'msTransform'; _.transformType = '-ms-transform'; _.transitionType = 'msTransition';
  ...
  Slick.prototype.setSlideClasses = function(index) {
    const _ = this; let centerOffset; let allSlides; let indexOffset; let remainder;

    allSlides = _.$slider.find('.slick-slide').removeClass('slick-active slick-center slick-current').attr('aria-hidden', 'true'); _.$slides.eq(index).addClass('slick-current');
    ...
    if (_.options.centerMode === true) {
      ...
      if (_.options.infinite === true) {
        if (index >= centerOffset && index <= _.slideCount - 1 - centerOffset) {
          ...
          indexOffset = _.options.slidesToShow + index; allSlides.slice(indexOffset - centerOffset + 1 + evenCoef, indexOffset + centerOffset + 2).addClass('slick-active').attr('aria-hidden', 'false');
        }
      ...
      if (index >= 0 && index <= _.slideCount - _.options.slidesToShow) {
        ...
        remainder = _.slideCount % _.options.slidesToShow; indexOffset = _.options.infinite === true ? _.options.slidesToShow + index : index;
  ...
  Slick.prototype.setupInfinite = function() {
    const _ = this; let i; let slideIndex; let infiniteCount;
  ...
  Slick.prototype.selectHandler = function(event) {
    const _ = this; const targetElement = $(event.target).is('.slick-slide') ? $(event.target) : $(event.target).parents('.slick-slide'); let index = parseInt(targetElement.attr('data-slick-index'));
  ...
  Slick.prototype.slideHandler = function(index, sync, dontAnimate) {
    let targetSlide; let animSlide; let oldSlide; let slideLeft; let targetLeft = null; const _ = this; let navTarget;
    ...
    if (_.options.asNavFor) {
      navTarget = _.getNavTarget(); navTarget = navTarget.slick('getSlick');
  ...
  Slick.prototype.swipeDirection = function() {
    ...
    xDist = _.touchObject.startX - _.touchObject.curX; yDist = _.touchObject.startY - _.touchObject.curY; r = Math.atan2(yDist, xDist); swipeAngle = Math.round(r * 180 / Math.PI);
  ...
  Slick.prototype.swipeEnd = function(event) {
    const _ = this; let slideCount; let direction;

    _.dragging = false; _.swiping = false;

    if (_.scrolling) {
      _.scrolling = false;

      return false;
    }_.interrupted = false; _.shouldClick = !(_.touchObject.swipeLength > 10);
    ...
    if (_.touchObject.swipeLength >= _.touchObject.minSwipe) {
      ...
      switch (direction) {
        case 'left':case 'down':slideCount = _.options.swipeToSlide ? _.checkNavigable(_.currentSlide + _.getSlideCount()) : _.currentSlide + _.getSlideCount(); _.currentDirection = 0; break; case 'right':case 'up':slideCount = _.options.swipeToSlide ? _.checkNavigable(_.currentSlide - _.getSlideCount()) : _.currentSlide - _.getSlideCount(); _.currentDirection = 1; break; default:
      }
  ...
  Slick.prototype.swipeHandler = function(event) {
    ...
    switch (event.data.action) {
      case 'start':_.swipeStart(event); break; case 'move':_.swipeMove(event); break; case 'end':_.swipeEnd(event); break;
    }
  };

  Slick.prototype.swipeMove = function(event) {
    const _ = this; const edgeWasHit = false; let curLeft; let swipeDirection; let swipeLength; let positionOffset; let touches; let verticalSwipeLength;
    ...
    if (event.originalEvent !== undefined && _.touchObject.swipeLength > 4) {
      _.swiping = true; event.preventDefault();
  ...
  Slick.prototype.updateArrows = function() {
    const _ = this; let centerOffset;

    centerOffset = Math.floor(_.options.slidesToShow / 2);

    if (_.options.arrows === true && _.slideCount > _.options.slidesToShow && !_.options.infinite) {
      _.$prevArrow.removeClass('slick-disabled').attr('aria-disabled', 'false'); _.$nextArrow.removeClass('slick-disabled').attr('aria-disabled', 'false');
  ...
  Slick.prototype.updateDots = function() {
    ...
    if (_.$dots !== null) {
      _.$dots.find('li').removeClass('slick-active').end(); _.$dots.find('li').eq(Math.floor(_.currentSlide / _.options.slidesToScroll)).addClass('slick-active');
    }
  };
  ...
  $.fn.slick = function() {
    const _ = this; const opt = arguments[0]; const args = Array.prototype.slice.call(arguments, 1); const l = _.length; let i; let ret;
